...
7. You can also add or edit a Topic Note, rename the Topic, retry setup, or delete only the Topic record from the action rail. A Topic Note is shown in yellow after the Topic name. It is one line and at most 200 characters; saving an empty Note removes it. **Rename Topic** opens a prompt for a new display name; it changes only the Topic name, never its Branch, Worktree, or repository.
