...
5. Select a Topic to open the action rail. The first available action has focus. Press `n` from the Topic list to add or edit its Topic Note. Press `m` to open or focus its resumable Main Agent directly. Press `o` to focus its i3 workspace directly. Press `p` to open its pull request in a browser. Press `r` to refresh pull request state now, ahead of the daemon poll cadence. Press `Shift+J` to Unfocus the selected Topic and `Shift+K` to Focus it: Focused Topics stay in the upper part, Unfocused ones sit below a blank separator, and the selection follows the moved Topic. New Topics start Focused.
...
7. You can also add or edit a Topic Note, rename the Topic, retry setup, or delete only the Topic record from the action rail. A Topic Note is shown in yellow in the **Note** column directly after the Topic title. The compact layout keeps it inline after the title. It is one line and at most 200 characters; saving an empty Note removes it. **Rename Topic** opens a prompt for a new display name; it changes only the Topic name, never its Branch, Worktree, or repository.

Topic names can form a visual hierarchy with the exact `>` separator. When the exact parent Topic
exists in the same Focus part, the list keeps the family together and replaces repeated parent
prefixes with tree connectors. An active child moves its family but stays below its parent. This
display does not change the stored Topic names.
