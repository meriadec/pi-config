...
move the complete family by one Partition arrangement step and keep the selection. A family first
leaves a shared Partition for a new adjacent Partition, then enters the next Partition with another
keypress. Partition order is the dominant sort key; Main Agent activity and names keep their existing
sort behavior inside each Partition. New root Topics join the first Partition. **Add Child Topic** in the
