...
export function buildParentLaunchMessage(
  ...
  const mode = options.handoffMode === "fork" ? "forked from parent conversation" : "isolated";
...
export function buildChildCompletionMessage(jobId: string, result: string): string {
  return `Delegation Result written for ${jobId}.\n\nResult:\n\n${result}`;
}
