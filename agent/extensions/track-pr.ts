...
function collectCopilotEvents(
  node: unknown,
  events: Array<{ kind: string; login: string; when?: string }>,
  kind: string,
) {
  ...
  const when =
    "submittedAt" in node && typeof node.submittedAt === "string"
      ? node.submittedAt
...
async function getCopilotStatus(pr: PullRequestRef, cwd: string): Promise<CopilotStatus> {
  ...
  const hasPendingCopilotRequest = Boolean(
    pullRequest.reviewRequests?.nodes?.some((node) =>
      isCopilotLogin(loginOfReviewer(node.requestedReviewer)),
    ),
  );

  const events: Array<{ kind: string; login: string; when?: string }> = [];
  for (const review of pullRequest.latestReviews?.nodes ?? [])
    collectCopilotEvents(review, events, "review");
  for (const comment of pullRequest.comments?.nodes ?? [])
    collectCopilotEvents(comment, events, "comment");
  for (const thread of pullRequest.reviewThreads?.nodes ?? []) {
    for (const comment of thread.comments?.nodes ?? [])
      collectCopilotEvents(comment, events, "review-thread comment");
  }
  ...
  try {
    while (!job.cancelled) {
      ...
      if (status.finished) {
        clearSpinner(ctx);
        ctx.ui.notify(
          `Copilot review finished for #${pr.number}: ${status.activitySummary}. Launching pr-review-triage.`,
          "info",
        );
      ...
      if (Date.now() - startedAt >= DEFAULT_TIMEOUT_MS) {
        throw new Error(
          `Timed out after ${DEFAULT_TIMEOUT_MS / 60_000} minutes waiting for Copilot on ${pr.url}`,
        );
      }
    ...
    clearSpinner(ctx);
    ctx.ui.notify(
      `track-pr failed: ${error instanceof Error ? error.message : String(error)}`,
      "error",
    );
  } finally {
