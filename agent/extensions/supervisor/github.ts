...
export class GitHubClient {
  ...
  async listNotifications(options: { all?: boolean } = {}): Promise<GitHubNotificationItem[]> {
    ...
    const result = await this.exec(
      "gh",
      ["api", `notifications?${all}per_page=${NOTIFICATION_LIMIT}`],
      {
    ...
    try {
      ...
      throw new Error(
        `failed to parse gh notifications JSON: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    const items = parsed
      ...
      .filter((item): item is GitHubNotificationItem => item !== undefined);
  ...
  private async hydratePullRequestInfo(items: GitHubNotificationItem[]): Promise<void> {
    const pullRequests = items
      .map((item, index) => ({ item, index, ref: parsePullRequestApiUrl(item.subject.url) }))
      .filter(
        (entry): entry is { item: GitHubNotificationItem; index: number; ref: PullRequestRef } =>
          entry.ref !== undefined,
      );
  ...
  async markDone(threadId: string): Promise<void> {
    const result = await this.exec(
      "gh",
      ["api", "-X", "DELETE", `notifications/threads/${threadId}`],
      {
        timeout: ACTION_TIMEOUT_MS,
      },
    );
...
function assertOk(result: ExecResult, message: string): void {
  ...
  const details = (
    result.stderr ||
    result.stdout ||
    (result.killed ? "command timed out" : "unknown error")
  ).trim();
