	...
	private renderColumnHeader(width: number): string {
		return this.dim(
			this.theme.bold(
				truncateToWidth(
					formatColumns(
						{ signal: "", age: "AGE", reason: "REASON", type: "TYPE", repo: "REPO", author: "AUTHOR", title: "TITLE" },
						false,
						width,
					),
					width,
...
function columnLayout(width: number): { signal: number; age: number; reason: number; type: number; repo: number; author: number; title: number } {
	const prefix = 2;
	const gaps = 6;
	const layout = { signal: 2, age: 4, reason: 16, type: 5, repo: 22, author: 14, title: 10 };

	let overflow =
		prefix + gaps + layout.signal + layout.age + layout.reason + layout.type + layout.repo + layout.author + layout.title - width;
	...
	layout.title = Math.max(
		1,
		width - prefix - gaps - layout.signal - layout.age - layout.reason - layout.type - layout.repo - layout.author,
	);
	return layout;
}
...
function notificationSignal(item: GitHubNotificationItem): { emoji: string; label: string } {
	...
	if (pullRequest?.merged || (pullRequest?.state && pullRequest.state !== "OPEN")) return { emoji: "✅", label: "merged/closed" };
