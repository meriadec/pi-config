...
- [x] A deterministic, agent-runnable test reproduces the long-wait cancellation path and asserts the reported symptom.
- [ ] The cancellation source is preserved across the process, daemon protocol, WorkClient, and Pi tool boundaries.
- [x] A cancelled result includes a specific reason, the last completed phase, and an actionable retry or recovery step.
- [x] Semantic progress shows the current provisioning phase without exposing raw Setup command output or sensitive paths.
- [x] An explicit timeout is documented and returns a timeout result, not a generic cancellation result.
- [x] Cancellation does not leave an unreported Topic, Branch, Worktree, or false `ready` state.
- [ ] Any intentional partial state is reconciled on retry or daemon restart and is reported to the caller.
- [x] Tests cover cancellation before request acceptance, during provisioning, and after durable Topic creation.
...
- The Pi tool waits for a terminal provisioning result and has a six-hour request timeout.
- A caller abort signal closes the WorkClient wait and returns a generic cancellation result.
- The daemon operation can continue after the client stops waiting.
- Semantic progress and timeout-specific protocol errors already exist.
- Timed-out retries and daemon recovery have integration coverage.
- No matching prior rejection or duplicate implementation exists.

**What we still need from the reporter:**

- Give the approximate request start and cancellation times.
- Give all progress text shown before cancellation.
- Say whether another user message was sent while the tool was running.
- Provide daemon logs for that time, with sensitive paths or credentials removed.
- Give the Topic list and relevant Branch or Worktree state after cancellation.
- Confirm whether the Source checkout and Base checkout used one Git common directory.

> *This was generated by AI during triage.*

## Treatment Notes

A deterministic unit test reproduced the generic cancellation in less than one second. A controlled
caller abort produced the reported result while the tool waited for provisioning. The adapter
discarded both the abort source and its last semantic phase.

The treatment now:

- identifies the result as `tool-call-cancelled`;
- includes the last semantic phase;
- distinguishes cancellation before and after a daemon request;
- warns that daemon provisioning can continue after the tool stops waiting;
- tells the user to inspect `/work` before retrying the repository and Branch;
- reports WorkClient request deadlines as `request-timeout`;
- documents the six-hour request deadline;
- verifies through the real daemon that provisioning reaches `ready` after tool cancellation.

The original reason for the long wait and the actor that aborted the Pi tool call are still unknown.
The issue stays in `needs-info` until the reporter supplies the requested runtime evidence.
